...
import { TERMS_VERSION, getLegalTerms } from '@/lib/estimateTerms';
...
export async function approveEstimate(token, opts = {}) {
  ...
  let termsVer = TERMS_VERSION; try { termsVer = (await getLegalTerms(sb, 'work_authorization')).version || TERMS_VERSION; } catch (_) {}
  const consentText = `I, ${name}, approve this estimate of $${total.toLocaleString()} from Clog Busterz Plumbing, authorize the work described, and agree to the Clog Busterz Work Authorization & Terms and Conditions (${termsVer}).`;
