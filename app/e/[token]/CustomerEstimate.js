...
import { approveEstimate, askQuestion, requestDeposit, declineEstimate, chooseTier } from './actions';
  ...
  const TierCard = (t, idx) => {
    ...
    return (
      <div key={t.key} style={{
        ...
        {t.warranty && <div style={{ fontSize: 12.5, color: GREEN, fontWeight: 700, marginTop: 8 }}>🛡 {t.warranty}</div>}
      ...
      {hasLadder && !closed && !done ? (
        ...
        <>
          ...
          <div style={{ ...card, background: SURF2 }}>
            ...
            {est.warranty && <div style={{ fontSize: 12, color: MUTE, marginTop: 10 }}>🛡 {est.warranty}</div>}
          </div>
