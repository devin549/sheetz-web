...
export default function CollectPay({ jobId, defaultAmount, tel }) {
  ...
  const emailIt = () => { if (!link) return; setLinkMsg(null); start(async () => { const r = await emailPayLink(jobId, { url: link.url, amountDollars: Number(link.baseDollars) || 0, extraEmail: extraEmail.trim() }); setLinkMsg(r); }); };
