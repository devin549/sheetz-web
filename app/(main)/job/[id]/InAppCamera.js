...
export default function InAppCamera({ label, onCapture, onClose, video = false }) {
    ...
    <div onClick={close} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.88)', zIndex: 80, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 14 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 520, display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {err ? (
          ...
          clipUrl ? (
            <>
              <video src={clipUrl} controls playsInline style={{ width: '100%', borderRadius: 14, maxHeight: '60vh', background: '#000' }} />
          ...
          <>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={usePhoto} className="btn" style={{ flex: 2, padding: 14, fontSize: 15 }}>✓ Use this photo</button>
            </div>
          </>
        ) : (
          <>
