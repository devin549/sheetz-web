...
import { generateIdeas, draftIdea, setIdeaStatus } from './actions';
            ...
            {byStatus[k].map((it) => (
              <div key={it.id} className="card">
                ...
                <div style={{ display: 'flex', gap: 6, marginTop: 9, flexWrap: 'wrap', alignItems: 'center' }}>
                  ...
                  {it.status !== 'published' && <button onClick={() => mark(it.id, 'published')} disabled={pending} className="pill" style={{ cursor: 'pointer', color: 'var(--green)' }}>✓ Published</button>}
