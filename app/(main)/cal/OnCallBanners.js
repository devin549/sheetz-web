...
export default function OnCallBanners({ windows = [] }) {
  ...
  // Acknowledge: open the Google Calendar add-link SYNCHRONOUSLY on the tap (so the popup isn't blocked) —
  // the on-call window lands on their calendar with a reminder — then record the ack so "I forgot" is gone.
  const ack = (w) => {
    if (w.calLink && typeof window !== 'undefined') { try { window.open(w.calLink, '_blank', 'noopener'); } catch (_) {} }
    setAcked((a) => ({ ...a, [w.id]: true }));
    start(() => { acknowledgeOnCall(w.id).catch(() => setAcked((a) => ({ ...a, [w.id]: false }))); });
  };
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 14 }}>
      {windows.map((w) => {
        ...
        return (
          <div key={w.id} className={done ? undefined : 'oncall-pulse'} style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', background: 'rgba(156,100,244,0.07)', border: '1px solid #9c64f4', borderRadius: 12, padding: '12px 16px' }}>
            ...
            {done ? (
              <span style={{ background: 'rgba(76,175,80,0.15)', color: 'var(--green-bright)', border: '1px solid #4caf50', borderRadius: 10, padding: '8px 14px', fontSize: 12, fontWeight: 800 }}>✓ Acknowledged · 📅 on your calendar</span>
            ) : (
              <button onClick={() => ack(w)} disabled={pending}
