...
export default async function ToolPurchases() {
  ...
  let plans = [];
  try {
    const { data, error } = await sb.from('tool_purchases').select('id, tool_name, tech_name, purchase_cents, weekly_pct, weekly_cents, paid_cents, vendor, status, started_on').order('status', { ascending: true }).order('started_on', { ascending: false }).limit(100);
    if (error) {
