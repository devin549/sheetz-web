...
export default async function ToolAccountability() {
  ...
  let events = [];
  try {
    const { data, error } = await sb.from('tool_events').select('tool_id, tool_name, event, holder_name, by_name, cost_cents, note, created_at').order('created_at', { ascending: false }).limit(200);
    if (error) {
