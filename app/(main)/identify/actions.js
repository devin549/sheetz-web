...
const STOP = new Set(['the', 'and', 'for', 'with', 'inch', 'new', 'set', 'kit', 'pack', 'genuine', 'oem', 'replacement', 'parts', 'part', 'home', 'depot', 'lowes', 'amazon', 'ebay', 'walmart']);
...
async function matchFixes(sb, guess, showCost) {
  ...
  let aliasByItem = {};
  try { const { data } = await sb.from('pricebook_item_aliases').select('item_id, phrase').eq('active', true); (data || []).forEach((a) => { (aliasByItem[a.item_id] = aliasByItem[a.item_id] || []).push(a.phrase); }); } catch (_) {}
