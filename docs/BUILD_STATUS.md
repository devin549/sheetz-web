...
| Accounts Receivable | /past-due | ✅ live | **QuickBooks-grade**: A/R aging summary buckets, search (name/CB#/phone), aging-filter chips, sort, collapsible per-customer cards, **Mark paid** (financial roles). ⏳ customer statement PDF, payment log (who/when/method, partials), Stripe pay-link, due-date aging (vs invoice-date), date-range filter |
