...
export const ROLES = {
  admin: {
    ...
    perms: {
      ...
      qaReview: true, qaOverride: true,
      // shell-reorg perm keys (deny-by-default for any role that doesn't list them)
      ownerSettings: true, closeoutOverride: true, closeFailedQa: true, manageMemberships: true, growth: true,
    },
  },
  dispatcher: {
  ...
  foreman: {
    ...
    perms: {
      ...
      qaReview: true, closeoutOverride: true, closeFailedQa: true, seeOwnPayOnly: true,
    },
  },
  tech: {
...
ROLES.gm = {
  ...
  perms: { ...ROLES.dispatcher.perms, seeRevenue: true, seeFinancials: true, manageUsers: true, seeReports: true, qaReview: true, qaOverride: true, closeoutOverride: true, closeFailedQa: true, manageMemberships: true, growth: true },
};
ROLES.om = {
  id: 'om', label: 'Office Manager', short: 'Tracey', color: 'oklch(66% 0.13 165)',
  perms: { ...ROLES.csr.perms, seeRevenue: true, seeReports: true, manageUsers: true, editSchedule: true, manageMemberships: true },
};
ROLES.accounting = {
  id: 'accounting', label: 'Accounting', short: 'Ashley', color: 'oklch(60% 0.10 295)',
  perms: { ...ROLES.viewer.perms, seeFinancials: true, seeReports: true, collectPayment: true, manageMemberships: true },
};
ROLES.fs = {
  id: 'fs', label: 'Field Supervisor', short: 'Crew + quality', color: 'oklch(64% 0.13 50)',
  perms: { ...ROLES.dispatcher.perms, seeRevenue: false, seeFinancials: false, seeReports: true, manageUsers: false, qaReview: true, qaOverride: true, closeoutOverride: true, closeFailedQa: true },
};
ROLES.sales = {
  id: 'sales', label: 'Sales', short: 'Estimates + pipeline', color: 'oklch(64% 0.16 145)',
  perms: { ...ROLES.csr.perms, seeRevenue: true, seeReports: true, createJobs: true, manageMemberships: true, growth: true },
};
ROLES.marketing = {
  id: 'marketing', label: 'Marketing', short: 'Campaigns + leads', color: 'oklch(66% 0.16 330)',
  perms: { ...ROLES.viewer.perms, seeRevenue: false, seeFinancials: false, seeReports: true, createJobs: true, contactCustomer: true, growth: true, manageMemberships: true },
};
ROLES.shop = {
...
// Helper — rides along & assists on a TECH's work order (CB_Dispatch_Helper_v1). NOT a tech:
// no close-out, no payment collection, no commission, no estimates, no pricing, and (per Devin)
// NO money/pay/AR at all — not even their own pay on the field device. Their "day" is the PAIRED
// TECH's jobs (office pairs helper↔tech daily in _DB_HelperAssign). Most-limited field seat; the
// phone-only HelperShell is their entire surface.
ROLES.helper = {
  id: 'helper', label: 'Helper', short: 'Ride-along assist', color: 'oklch(66% 0.09 200)',
  perms: {
    seeOwnOnly: true, seeAllTechs: false, seeAllJobs: false, seeRevenue: false, seeQueue: false,
    assignJobs: false, reassignJobs: false, editSchedule: false,
    createJobs: false, deleteJobs: false, changeStatus: false, contactCustomer: false,
    seeReports: false, manageUsers: false, seeFinancials: false, collectPayment: false,
    seeOwnPayOnly: false, // helpers do not see money on the field device
    helperAssist: true,    // can attach assist photos to the paired tech's job
  },
};
