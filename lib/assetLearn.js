...
// Owned-equipment roster (model + how many). Best-effort; empty until mig 146 is run.
export async function fleetRoster(sb) {
  try { const { data } = await sb.from('equipment_fleet').select('model, model_key, count, kind, note').eq('active', true); return data || []; }
  catch (_) { return []; }
}

// Per-model picture: how many we OWN vs the recent distinct locations posted — so Hank can say "2 of your 4
// 17Gs were posted recently — 426 E Broadway + 2501 Mansion View; 2 haven't come up." Matches a sighting to a
// model when their spaceless keys prefix-overlap ("17g" ⊃ "17g3" for a labeled unit).
export async function fleetSummary(sb) {
  const [roster, sightings] = await Promise.all([fleetRoster(sb), recentSightings(sb, { limit: 80 })]);
  return roster.map((m) => {
    const mk = spaceless(m.model_key);
    const hits = sightings.filter((s) => { const sk = spaceless(s.asset_key); return sk === mk || sk.startsWith(mk) || mk.startsWith(sk); });
    const locations = hits.map((h) => ({ location: h.location, holder: h.holder, by: h.by_name, when: h.created_at, said: h.asset }));
    return { model: m.model, count: m.count, kind: m.kind, seen: locations.length, locations: locations.slice(0, Math.max(m.count, 4)) };
  });
}
