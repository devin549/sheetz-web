...
// Latest known row per asset (newest first) — for Hank's context + a "what do we know" view.
export async function latestAssetLocations(sb, { limit = 40 } = {}) {
  try {
    ...
    for (const r of (data || [])) { if (seen.has(r.asset_key)) continue; seen.add(r.asset_key); out.push(r); if (out.length >= limit) break; }
    return out;
  } catch (_) { return []; }
}

// Best single match for "where's the <query>?" — newest row whose asset/key contains the query.
export async function findAssetLocation(sb, query) {
  const q = String(query || '').trim(); if (!q) return null;
  const all = await latestAssetLocations(sb, { limit: 200 });
  const qk = spaceless(q);
  return all.find((r) => spaceless(r.asset).includes(qk) || qk.includes(spaceless(r.asset)) || String(r.asset).toLowerCase().includes(q.toLowerCase())) || null;
}
