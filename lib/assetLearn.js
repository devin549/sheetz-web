...
// Owned-equipment roster — one row per physical machine (tag_code-ready, with scan snapshot once mig 147 is run).
// select('*') so it works whether or not the scan columns exist yet. Best-effort; empty until mig 146 is run.
export async function fleetRoster(sb) {
  try { const { data } = await sb.from('equipment_fleet').select('*').eq('active', true).order('unit_label'); return data || []; }
  catch (_) { return []; }
}
...
export async function fleetSummary(sb) {
  ...
  for (const u of units) {
    ...
    byModel.get(key).units.push({ label: u.unit_label, tagged: !!u.tag_code, status: u.status || 'in', heldBy: u.held_by || null, location: u.location || null, lat: u.lat ?? null, lng: u.lng ?? null, scannedAt: u.scanned_at || null, scannedBy: u.scanned_by || null });
  }
