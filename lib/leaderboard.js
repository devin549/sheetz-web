...
export function nyWeekWindow(now) {
...
// Weekly award ELIGIBILITY / DQ triggers (ported from the Races "all awards live · zero strikes" panel).
// Any late arrival this week = DQ. Late = real from jobs (started_at > scheduled_at + 5 min grace).
// Callbacks come from open photo-correction QA holds on this week's jobs. Low-review / low-margin DQs
// need review + cost feeds (return 0 for now). Guarded + fail-soft.
export async function weeklyEligibility(sb, { techId, name }, nowMs) {
  try {
    const { startISO, endISO } = nyWeekWindow(Number.isFinite(nowMs) ? new Date(nowMs) : new Date());
    let q = sb.from('jobs').select('id, scheduled_at, started_at').gte('scheduled_at', startISO).lt('scheduled_at', endISO);
    q = techId ? q.eq('tech_id', techId) : q.ilike('tech_name', String(name || ''));
    const { data, error } = await q;
    if (error) return { available: false };
    const jobs = data || [];
    let late = 0;
    for (const j of jobs) { if (j.scheduled_at && j.started_at && Date.parse(j.started_at) > Date.parse(j.scheduled_at) + 5 * 60000) late++; }
    let callbacks = 0;
    try {
    ...
    const eligible = late === 0 && callbacks === 0;
    return { available: true, late, callbacks, lowReviews: 0, lowMargin: 0, eligible, strikes: late + callbacks };
