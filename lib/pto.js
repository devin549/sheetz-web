...
//   vacationDays      — this employee's allotment (5 = 1 wk default, 10 = 2 wks). Holidays stay a fixed 5/yr.
export function vacationStatus({ hireDate, timeOff = [], vacationPullDays = 0, vacationDays = VACATION_DAYS } = {}, now = new Date()) {
  const allotDays = (vacationDays == null || isNaN(Number(vacationDays))) ? VACATION_DAYS : Math.max(0, Number(vacationDays));
  const allotHours = allotDays * HOURS_PER_DAY;
  const since = daysSinceHire(hireDate, now);
  const earned = since != null && since >= 365;       // the allotment lands only after the 1-yr mark
  const grantHours = earned ? allotHours : 0;
...
// ── Holiday "work the day before AND after" guard ─────────────────────────────────────────────────────
// To get PAID for a holiday you must work the scheduled workday immediately before AND after it — no
// stretching a holiday into a long weekend by calling out. Weekends are skipped: a Saturday holiday means
// you must work FRIDAY and MONDAY; a Monday holiday means the prior FRIDAY and the next TUESDAY.
const iso = (d) => d.toISOString().slice(0, 10);
const isWeekend = (d) => d.getDay() === 0 || d.getDay() === 6;
function stepWorkday(d, dir) { const x = new Date(d); do { x.setDate(x.getDate() + dir); } while (isWeekend(x)); return x; }

// The two workdays a holiday is gated on. null if no date. { before, after } as 'YYYY-MM-DD'.
export function holidayGuardDays(holidayDate) {
  const h = asDate(holidayDate);
  if (!h) return null;
  return { before: iso(stepWorkday(h, -1)), after: iso(stepWorkday(h, +1)) };
}

// Is this specific holiday earned? Forfeited if the person was absent (call-out / no-show) on either guard
// day — they must actually WORK both. absenceDates = the 'YYYY-MM-DD' strings they missed.
export function holidayEarned(holidayDate, absenceDates = []) {
  const g = holidayGuardDays(holidayDate);
  if (!g) return { eligible: true, before: null, after: null, missed: [] };
  const set = new Set(Array.isArray(absenceDates) ? absenceDates : []);
  const missed = [g.before, g.after].filter((d) => set.has(d));
  return { eligible: missed.length === 0, before: g.before, after: g.after, missed };
}
