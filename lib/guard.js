...
// Resolve the signed-in user (or bounce to /login). Role + scope come from the profiles table
// (server-authoritative), falling back to auth user_metadata until a login is backfilled.
// Returns { user, role, profile } — profile carries tech_id / crew_id / active for scoping.
async function requireUser() {
  const supabase = createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) redirect('/login');
  const profile = await loadProfile(user);
  // Deactivated employee → out, even if their auth ban didn't take (defense-in-depth on profiles.active).
...
export async function requireRole(allowed) {
  const { user, role, profile } = await requireUser();
