...
import { fleetSummary, recentSightings } from '@/lib/assetLearn';
...
export async function hankContext(sb, opts = {}) {
  ...
  const fieldCrew = roster.filter((t) => t.active && FIELD_POSITIONS.includes(String(t.position || '').trim().toLowerCase())); // case-insensitive — 'Tech' ≠ 'tech' was dropping people
  ...
  try {
    // Customer name + address live on the related customers row, not a flat jobs column.
    const { data } = await sb.from('jobs').select('*, customers(name, address)').gte('scheduled_at', todayISO).limit(300);
    (data || []).forEach((j) => {
      const c = j.customers || {};
      const status = String(j.status || '').toLowerCase();
      const active = /scheduled|enroute|on_site|on site|dispatched/.test(status);
      const addr = j.address || c.address || j.street || '', city = j.city || j.address_city || '';
      ...
      jobsToday.push({ customer: j.customer_name || c.name || '', city, type: j.job_type || '', status: j.status || '', tech: j.tech_name || '', addr });
    });
  } catch (_) {}
  ...
  // Chat-learned asset locations (#general): per-model fleet picture (own-count vs recent locations) + the
  // raw recent sightings. We own MULTIPLES of some machines (4× 17G), so a model has several live locations.
  let fleet = [], assetLocations = [];
  try { fleet = await fleetSummary(sb); } catch (_) {}
  try { assetLocations = (await recentSightings(sb, { limit: 30 })).map((a) => ({ asset: a.asset, kind: a.kind, action: a.action, location: a.location, holder: a.holder, by: a.by_name, when: a.created_at })); } catch (_) {}
  ...
  return {
    ...
    fleet: fleet.length ? fleet : undefined,
    assetLocations: assetLocations.length ? assetLocations : undefined,
    assetNote: 'fleet = each equipment model, how many we OWN (count), and the recent distinct locations posted in #general (chat-learned). assetLocations = the raw recent sightings (machines/tools/keys/materials, newest first). We own MULTIPLES of some machines (e.g. 4× 17G), so "where\'s the 17G?" usually has SEVERAL answers — list each recent location + who moved it + how long ago, and if there are fewer locations than we own, say the rest haven\'t been posted lately (e.g. "2 of your 4 17Gs were posted — 426 E Broadway and 2501 Mansion View; the other two haven\'t come up"). If a unit # / nickname was posted, use it. If nothing\'s posted for an asset, say you haven\'t seen it — never invent a location.',
...
export async function runHank(sb, { autoPost = true, limit = 12, skipSync = false } = {}) {
  if (!skipSync) await syncDiscordCore(sb); // make sure the latest #sheetz chatter is in the feed (skip when the caller just synced)
  let unseen = [];
  try {
  ...
  // If the AI call failed, do NOT mark them seen — leave them for the next run (don't silently drop real help).
