...
export async function hankContext(sb) {
  ...
  // Tool registry — who currently holds each company tool (camera/auger/locator/jetter…).
  let tools = [];
  try {
    const { data } = await sb.from('tools').select('name, serial, assigned_to, status').limit(300);
    tools = (data || []).filter((t) => t.name).map((t) => ({
      tool: t.name,
      serial: t.serial || undefined,
      heldBy: t.assigned_to || null,            // null/empty = in the shop, not signed out
      status: t.status || '',                   // e.g. on_van | in_shop | repair | lost
    }));
  } catch (_) {}
  ...
  return {
    ...
    toolNote: 'tools[] is the company tool registry. heldBy = the tech who signed it out (null/empty = it’s in the shop). You can answer "who has the camera/auger/locator?" from this. You do NOT track WHICH physical shop (Richmond vs Lexington) a shop-held tool sits in — don’t claim a specific shop unless status says so. If a tool isn’t in the list, stay silent.',
  };
}
