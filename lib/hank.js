...
export async function hankContext(sb) {
  ...
  let crew = [];
  try {
    const { data } = await sb.from('techs').select('*').limit(200);
    crew = (data || []).map((t) => ({ name: t.name || t.full_name || '', position: t.position || t.role || '', active: t.active !== false }))
  ...
  try {
    ...
    (data || []).forEach((j) => {
      ...
      if (active) { openCount++; if (j.tech_name) busyTechs.add(String(j.tech_name).toLowerCase()); }
      jobsToday.push({ customer: j.customer_name || '', city: j.city || j.address_city || '', type: j.job_type || '', status: j.status || '', tech: j.tech_name || '', addr: j.address || j.street || '' });
    });
  } catch (_) {}
  jobsToday = jobsToday.slice(0, 40);

  // Rough availability: active crew NOT currently on an active job today.
  const freeCrew = crew.filter((t) => !busyTechs.has(t.name.toLowerCase())).map((t) => t.position ? `${t.name} (${t.position})` : t.name);
  ...
  return {
    crew: crew.map((t) => t.position ? `${t.name} (${t.position})` : t.name),
    likelyAvailable: freeCrew,
    jobsToday: { openActive: openCount, sample: jobsToday },
    shopStock: shopStock.length ? shopStock : undefined,
    note: 'Hank does NOT have tool-by-person tracking (who physically holds a camera/auger/locator) — that lives in the field app, not here. Stay silent if asked about who holds a specific tool.',
  };
}

const SYSTEM = [
  'You are Hank, a plumber-savvy teammate in Clog Busterz\' internal #sheetz team chat (plumbing company, Richmond + Lexington KY).',
  ...
  '5. You do not track who physically holds a specific tool — skip those.',
...
export async function runHank(sb, { autoPost = true, limit = 12 } = {}) {
  ...
  for (const r of replies) {
    if (autoPost) {
      const out = await postToDiscord(`🔧 Hank: ${r.reply}`);
      try { await sb.from('cb_comms').insert({ channel: 'discord', direction: 'out', to_addr: '#sheetz', from_name: 'Hank', body: r.reply, status: out.ok ? 'sent' : 'failed', error: out.ok ? null : out.error, reply_to: r.id, sent_by: 'Hank' }); } catch (_) {}
      if (out.ok) posted++;
    }
  }
...
export async function askHankCore(sb, question, { post = false } = {}) {
  ...
  try {
    res = await anthropic.messages.create({
      ...
      system: 'You are Hank, Clog Busterz\' plumber-savvy team assistant. Answer the question directly and briefly (1-3 sentences) using ONLY the JSON data + recent chat provided. Plain, friendly, plumber voice. If the data does not contain it, say briefly what you\'d need — do not guess.',
  ...
  if (post) {
    const out = await postToDiscord(`🔧 Hank: ${answer}`);
    try { await sb.from('cb_comms').insert({ channel: 'discord', direction: 'out', to_addr: '#sheetz', from_name: 'Hank', body: answer, status: out.ok ? 'sent' : 'failed', error: out.ok ? null : out.error, sent_by: 'Hank' }); } catch (_) {}
  }
