...
import { latestAssetLocations } from '@/lib/assetLearn';
...
export async function hankContext(sb, opts = {}) {
  ...
  // Chat-learned asset locations (#general): latest known where/who for machines, tools, keys, materials.
  let assetLocations = [];
  try { assetLocations = (await latestAssetLocations(sb, { limit: 40 })).map((a) => ({ asset: a.asset, kind: a.kind, action: a.action, location: a.location, holder: a.holder, by: a.by_name, when: a.created_at })); } catch (_) {}
  ...
  return {
    ...
    assetNote: 'assetLocations = the LATEST place each big asset/tool/key/material was reported in #general (chat-learned, newest per item). For "where\'s the 17G / mini ex / storage key?" answer from here: name the location + who moved it + how long ago (e.g. "426 E Broadway, Winchester — Ronnie dropped it 2 days ago"). If the asset isn\'t in the list, say you haven\'t seen it posted — do NOT guess.',
