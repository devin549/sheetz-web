...
export async function askHankCore(sb, question, { post = false, askerName = '' } = {}) {
  ...
  // Chat history for "where is it / who has it / did anyone say…" — skim a wide-enough window (with
  // timestamps) that Hank can find what someone said earlier, not just the last few lines.
  let history = [];
  try {
    const { data } = await sb.from('cb_comms').select('from_name, body, direction, created_at').eq('channel', 'discord').is('deleted_at', null).order('created_at', { ascending: false }).limit(50);
    history = (data || []).reverse()
      .filter((m) => String(m.body || '').trim())
      .map((m) => { const when = (() => { try { return new Date(m.created_at).toLocaleString([], { weekday: 'short', hour: 'numeric', minute: '2-digit' }); } catch { return ''; } })(); return `[${when}] ${m.from_name || (m.direction === 'in' ? 'crew' : 'office')}: ${m.body}`; });
  } catch (_) {}
  const anthropic = getAnthropic(HANK_ROLE);
  let res;
  try {
    res = await anthropic.messages.create({
      ...
      system: 'You are Pipe Wrench Hank, Clog Busterz\' plumber-savvy team assistant. Answer the question directly and briefly (1-3 sentences) using ONLY the JSON data + the chat history provided. For "where is X / who has X / did anyone say…" questions, SKIM the chat history and quote what the person said and when (e.g. "Chris said around 11:19 he\'s got the big reel, back this afternoon"). The tools[] registry is the source of truth for company tools; the chat history catches the rest. Plain, friendly, plumber voice. If it\'s in neither the data nor the chat, say briefly what you\'d need — do not guess.',
      messages: [{ role: 'user', content: `Data:\n${JSON.stringify(ctx)}\n\n#sheetz chat history (oldest→newest):\n${history.join('\n')}\n\nQuestion: ${String(question || '').trim()}` }],
    });
