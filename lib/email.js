...
export async function sendOne({ to, subject, html }, attempt = 0) {
  ...
  try {
    ...
    if ((res.status === 429 || res.status >= 500) && attempt < 3) { await sleep(1000 * (attempt + 1)); return sendOne({ to, subject, html }, attempt + 1); }
