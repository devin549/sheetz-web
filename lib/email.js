...
export async function sendOne({ to, subject, html, attachments }, attempt = 0) {
  ...
  try {
    const res = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({ from: FROM_EMAIL, to: [to], subject, html, ...(attachments ? { attachments } : {}) }),
    });
    if ((res.status === 429 || res.status >= 500) && attempt < 3) { await sleep(1000 * (attempt + 1)); return sendOne({ to, subject, html, attachments }, attempt + 1); }
    ...
    if (attempt < 2) { await sleep(800 * (attempt + 1)); return sendOne({ to, subject, html }, attempt + 1); }
