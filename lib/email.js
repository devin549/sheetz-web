...
// ⚠️ TEMPORARY OVERRIDE — send from the already-verified sheetzz.com so receipts/invoices actually deliver while
// clogbusterzplumbing.com is pending Resend verification (its Vercel EMAIL_FROM is set but the domain isn't
// verified yet, so every send 403s). Display name stays "Clog Busterz Plumbing". ➡️ ONCE clogbusterzplumbing.com
// is verified in Resend, DELETE this line to restore `process.env.EMAIL_FROM || …` and set EMAIL_FROM in Vercel to
// "Clog Busterz Plumbing <billing@clogbusterzplumbing.com>".
export const FROM_EMAIL = 'Clog Busterz Plumbing <billing@sheetzz.com>';
