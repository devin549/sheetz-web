...
export async function updateSession(request) {
  ...
  const domain = cookieDomainForHost(request.headers.get('host') || request.nextUrl.hostname);
    ...
    process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY,
    {
      cookies: {
        ...
        setAll(cookiesToSet) {
          ...
          cookiesToSet.forEach(({ name, value, options }) => supabaseResponse.cookies.set(name, value, domain ? { ...options, domain } : options));
        },
      },
    }
  );
  ...
  // Static public assets (logo, etc.) must be reachable without auth — incl. by customers' mail
  // clients loading the logo in emailed statements.
  const isAsset = /\.(?:jpg|jpeg|png|gif|svg|webp|ico|css|js|txt|woff2?|map)$/i.test(path);
