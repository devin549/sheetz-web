...
const LIVE = {
  ...
  topCustomers: '/top-customers', profitTruth: '/revenue', askBoard: '/ask',
...
// Build the role's cockpit for a shell: { pinned, groups, account }. shell=null → all shells (role-only,
// used by canSee for security). shell='office'|'tech'|'shop' → only that shell's nav.
export function navGroupsFor(role, shell = null) {
  const pinned = PINNED.filter((p) => (p.roles.includes('*') || roleOk(p.roles, role)) && inShell(p.key, shell)).map(decorate);
  const groups = GROUPS
    .filter((g) => roleOk(g.roles, role) && inShell(g.id, shell))
