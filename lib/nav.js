...
export const NAV = [
  ...
  // My Day = the TECH IPAD home — field only (techs + helpers out on the iPad), NOT office/owner.
  { href: '/my-day',    label: 'My Day',    icon: '📋', show: (r) => can(r, 'seeOwnOnly') },
  // QA / Closeouts = the supervisor command center (verify the work). FS/foreman/GM/owner.
  { href: '/supervisor/jobs', label: 'QA / Closeouts', icon: '✅', show: (r) => can(r, 'qaReview') },
  // Hank = the plumber's-brain AI Q&A (ported from the iPad). All internal staff, not viewer/customer.
  { href: '/hank',      label: 'Hank',      icon: '🪠', show: (r) => !['viewer', 'customer'].includes(String(r || '').toLowerCase()) },
  // Estimate = Good/Better/Best price-book presenter (ported). PRESENT_ROLES minus helper —
  // helpers don't own estimates on the live iPad (#nav-estimates is hidden for cb-helper).
  { href: '/estimate',  label: 'Estimate',  icon: '🧾', show: (r) => ['owner', 'admin', 'gm', 'om', 'accounting', 'fs', 'foreman', 'tech', 'dispatcher', 'csr', 'sales', 'marketing'].includes(String(r || '').toLowerCase()) },
