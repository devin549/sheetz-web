import { getSupabaseAdmin, isAdminConfigured } from '@/lib/supabaseAdmin';

// Loads the employee profile for a signed-in auth user — the server-authoritative source of
// role + scope (tech_id / crew_id / active). Falls back to auth user_metadata when the profiles
// table or row isn't there yet, so this is safe to ship before every login is backfilled.
//
// Returns: { role, tech_id, crew_id, active, name, email }
export async function loadProfile(user) {
  const meta = (user && user.user_metadata) || {};
  const fallback = {
    // SECURITY: role is NEVER taken from user_metadata — a logged-in user can self-edit their own metadata
    // (supabase.auth.updateUser) and would otherwise self-promote to owner. profiles.role is the ONLY
    // authority; no row / null role ⇒ lowest privilege ('viewer', read-only). Real employees get a profiles
    // row from Team management — backfill anyone relying on metadata, or they drop to view-only.
    role: 'viewer',
    tech_id: meta.tech_id || null,
    crew_id: meta.crew_id || null,
    active: true,
    name: meta.name || (user && user.email) || '',
    email: (user && user.email) || '',
    fieldMode: false,
    shopMode: false,
    roastLevel: 'PG',
    roastLocked: false,
    prefs: {},
    onboardedAt: null,
    ccPinSet: false,
    ccPinReady: false,
    ccLockUntil: null,
    ipadPinSet: false,
    ipadPinReady: false,
    ipadLockUntil: null,
    phone: '',
    licenseReady: false,
    licenseOnFile: false,
    licenseExpiry: null,
    licenseState: null,
    licenseName: null,
  };
  if (!user || !isAdminConfigured) return fallback;

  try {
    const sb = getSupabaseAdmin();
    // select('*') so new columns (field_mode_enabled/shop_mode_enabled) survive before the migration runs.
    const { data, error } = await sb
      .from('profiles')
      .select('*')
      .eq('user_id', user.id)
      .maybeSingle();
    if (error || !data) return fallback; // table/row missing → metadata fallback (no lockout)
    return {
      role: data.role || fallback.role,
      tech_id: data.tech_id ?? fallback.tech_id,
      crew_id: data.crew_id ?? fallback.crew_id,
      active: data.active !== false,
      name: data.name || fallback.name,
      email: data.email || fallback.email,
      photoUrl: data.photo_url || null,
      homeAddress: data.home_address || null,
      homeLat: typeof data.home_lat === 'number' ? data.home_lat : null,
      homeLng: typeof data.home_lng === 'number' ? data.home_lng : null,
      fieldMode: data.field_mode_enabled === true,
      shopMode: data.shop_mode_enabled === true,
      roastLevel: ['PG', 'PG-13', 'R'].includes(data.roast_level) ? data.roast_level : 'PG',
      roastLocked: data.roast_locked === true,
      prefs: (data.prefs && typeof data.prefs === 'object') ? data.prefs : {},
      onboardedAt: data.onboarded_at || null,
      ccPinSet: !!data.cc_pin_hash,
      ccPinReady: ('cc_pin_hash' in data), // column present → migration 76 applied (gate may activate)
      ccLockUntil: data.cc_pin_lock_until || null,
      ipadPinSet: !!data.ipad_pin_hash,
      ipadPinReady: ('ipad_pin_hash' in data), // migration 78 applied
      ipadLockUntil: data.ipad_pin_lock_until || null,
      phone: data.phone || '',
      licenseReady: ('license_on_file' in data), // migration 79 applied
      licenseOnFile: data.license_on_file === true,
      licenseExpiry: data.license_expiry || null,
      licenseState: data.license_state || null,
      licenseName: data.license_name || null,
    };
  } catch {
    return fallback;
  }
}
