...
const RECEIPT_PROMPT = `You are reading a photo of a purchase RECEIPT or invoice for plumbing parts/supplies. Return STRICT JSON only, no prose:
{"vendor":"<store/supplier name, e.g. Ferguson, Home Depot, Winsupply>","total":<grand total number, no $>,"date":"<YYYY-MM-DD or empty>","items":<count of line items, integer>,"confidence":"high|medium|low","notes":"<short note, e.g. tax included>"}
If a field isn't legible, use "" (or 0 for total/items). The total is the FINAL amount paid (incl. tax).`;
