...
export async function getCardPresentStatus(paymentIntentId) {
  ...
  try {
    ...
    // base_cents = the amount that applies to the invoice (total minus the card fee). Lets the caller SUBTRACT
    // the real charged amount from the balance instead of zeroing it (partial/deposit reader charges).
    const baseCents = Number(pi.metadata && pi.metadata.base_cents) || Number(pi.amount_received) || Number(pi.amount) || 0;
    return { ok: true, status: pi.status, paid, baseCents, lastError: (pi.last_payment_error && pi.last_payment_error.message) || null };
  } catch (e) { return { ok: false, error: String((e && e.message) || e).slice(0, 200) }; }
}
