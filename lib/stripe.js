...
// Is the client-side publishable key set? Needed for the in-app "key in card" terminal (Stripe Elements).
export const isStripePublishableConfigured = () => !!process.env.NEXT_PUBLIC_STRIPE_PUBLISHABLE_KEY;

// Create a KEYED (MOTO — mail-order/telephone-order) card PaymentIntent: staff types the card by hand for a
// phone/in-hand payment. moto:true tells Stripe the card isn't customer-present (skips SCA, adjusts fraud
// rules). Card data is entered via Stripe Elements client-side and never touches our server. The client
// confirms with the returned client_secret. Returns { ok, clientSecret, id, totals } — never throws.
export async function createKeyedCardIntent({ amountCents, invoiceNumber, customerName, invoiceId, customerId }) {
  const stripe = getStripe();
  if (!stripe) return { ok: false, error: 'STRIPE_SECRET_KEY not set' };
  const cents = Math.round(Number(amountCents) || 0);
  if (cents < 50) return { ok: false, error: 'Amount must be at least $0.50' };
  const fee = feeCentsFor(cents);
  const total = cents + fee;
  const md = { invoice_id: invoiceId || '', customer_id: customerId || '', invoice_number: invoiceNumber || '', customer_name: customerName || '', base_cents: String(cents), fee_cents: String(fee), method: 'card_keyed' };
  try {
    const pi = await stripe.paymentIntents.create({
      amount: total,
      currency: 'usd',
      payment_method_types: ['card'],
      payment_method_options: { card: { moto: true } }, // mail-order/telephone-order: staff-keyed, card-not-present
      description: `Clog Busterz Plumbing${invoiceNumber ? ` — Invoice ${invoiceNumber}` : ''}`,
      metadata: md,
    });
    return { ok: true, clientSecret: pi.client_secret, id: pi.id, baseCents: cents, feeCents: fee, totalCents: total };
  } catch (e) { return { ok: false, error: String((e && e.message) || e).slice(0, 200) }; }
}
