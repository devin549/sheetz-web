...
// Roll up a tech's (or the whole crew's) plans for the summary cards. Waived = "kept on van" company gear,
// so it's NOT charging and is excluded from the weekly/owed totals.
export function summarize(purchases = []) {
  const charging = purchases.filter((p) => p.status === 'active' && !p.waived);
  return {
    count: charging.length,
    weeklyCents: charging.reduce((s, p) => s + nextDeductionCents(p), 0),
    owedCents: charging.reduce((s, p) => s + remainingCents(p), 0),
    paidCents: purchases.reduce((s, p) => s + (Number(p.paid_cents) || 0), 0),
    onVanCount: purchases.filter((p) => p.waived && p.status !== 'closed').length,
  };
}
