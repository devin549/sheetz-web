...
// Reopen SNOOZED tasks whose snooze_until has passed. snooze_until was write-only dead schema (set but never
// read), so a snoozed worsening alert stayed buried forever. Run from the triggers cron before escalation.
export async function reopenSnoozedAlerts(sb, { nowISO } = {}) {
  sb = sb || getSupabaseAdmin();
  if (!sb) return { ok: false, reopened: 0 };
  const now = nowISO || new Date().toISOString();
  try {
    const { data, error } = await sb.from('tasks').update({ status: 'open' })
      .eq('status', 'snoozed').not('snooze_until', 'is', null).lt('snooze_until', now).select('id');
    if (error) return { ok: false, reopened: 0, msg: error.message };
    return { ok: true, reopened: (data || []).length };
  } catch (_) { return { ok: false, reopened: 0 }; }
}
