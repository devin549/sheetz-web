...
// Generic channel reader (used by the #general asset-location learner). Returns human messages newest-first,
// each with the replied-to message's text/author inline (`reply`) so "picked up and dropped at X" can resolve
// the asset from the message it answered. Needs the bot token; pass any channel id.
export async function fetchChannelMessages(channelId, limit = 40) {
  const token = process.env.DISCORD_BOT_TOKEN;
  if (!token || !channelId) return { ok: false, error: 'DISCORD_BOT_TOKEN / channelId not set', messages: [] };
  try {
    const r = await fetch(`https://discord.com/api/v10/channels/${channelId}/messages?limit=${Math.min(100, limit)}`, {
      headers: { Authorization: `Bot ${token}` }, cache: 'no-store',
    });
    if (!r.ok) { const t = await r.text().catch(() => ''); return { ok: false, error: `Discord ${r.status} ${t.slice(0, 120)}`, messages: [] }; }
    const raw = await r.json();
    const messages = (Array.isArray(raw) ? raw : [])
      .filter((m) => !m.webhook_id && !(m.author && m.author.bot))
      .map((m) => ({
        id: m.id,
        author: (m.author && (m.author.global_name || m.author.username)) || 'crew',
        content: String(m.content || '').slice(0, 1000),
        // The replied-to message (Discord inlines it on a reply) — the asset is often only in there.
        reply: m.referenced_message ? {
          author: (m.referenced_message.author && (m.referenced_message.author.global_name || m.referenced_message.author.username)) || '',
          content: String(m.referenced_message.content || '').slice(0, 500),
        } : null,
        at: m.timestamp,
      }));
