...
//   subtotal   = revenue − min(dispatch_fee, dispatch_fee_cap) − materialMarkedUp − sub_cost
//   commission = max(0, subtotal) × (commission_pct / 100)
//   premium    = materialMarkedUp × (premium% / 100)
//   jobPay     = commission + premium
// MATERIAL is marked up (2×/1.5×). A SUBCONTRACTOR cost is passed AT COST — no markup, no premium — and
// simply "taken off" the commissionable base. A sub line is PENDING until Accounting verifies it
// (sub_verified=true); pay still shows the number, flagged pending, so nothing finalizes unverified.
...
export function computeJobPay(job, commissionPct, st = CB_STRUCTURE) {
  ...
  const sub = Math.max(0, Number(job.sub_cost_cents) || 0);            // subcontractor — passed AT COST
  const subPending = sub > 0 && job.sub_verified !== true;             // Accounting must verify before final
  ...
  const subtotal = revenue - dispatch - materialMarkedUp - sub;        // sub deducted at cost ("we just take off")
  ...
  return { revenue, material, sub, subPending, dispatch, markup, premiumPct, materialMarkedUp, subtotal, commission, premium, jobPay: commission + premium };
}
...
export function computeWeeklyPay({ jobs = [], profile = {}, structure = CB_STRUCTURE, grants = [], hours = 0, ptoHours = 0 }) {
  ...
  const subDeduction = lines.reduce((s, l) => s + l.sub, 0);              // subcontractor cost, at cost
  const subPending = lines.some((l) => l.subPending);                     // any unverified sub this week
