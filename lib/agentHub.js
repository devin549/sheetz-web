...
export const AGENT_IDS = ['accounting-money', 'dispatch-ops', 'price-margin'];
...
export async function runClaudeAgent(agentId, snapshot) {
...
- Return strict JSON only with this shape:
{
  ...
  const response = await client.messages.create({
    ...
    messages: [
      {
        role: 'user',
        content: `Audit this snapshot for your mission. Return JSON only.\n\n${JSON.stringify(snapshot)}`,
      },
    ],
  });
...
export async function runCollaboratorRollup(agentReports, snapshot) {
...
export async function runAgentHub(options = {}) {
  ...
  const agentReports = await Promise.all(agentIds.map((id) => runClaudeAgent(id, snapshot)));
  const collaborator = options.collaborator === false
    ? null
    : await runCollaboratorRollup(agentReports, snapshot);
