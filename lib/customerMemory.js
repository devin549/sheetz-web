...
export async function loadCustomerMemory(sb, job) {
  ...
  // ── Equipment (lightweight: from equipment-kind photos — full equipment table is a later build) ──
  const eqPhotos = photos.filter((p) => isEquip(p.kind));
  const eqByJob = {}; eqPhotos.forEach((p) => { (eqByJob[p.job_id] = eqByJob[p.job_id] || []).push(p); });
  for (const jid of Object.keys(eqByJob).slice(0, 4)) {
    const j = jobs.find((x) => String(x.id) === String(jid));
    let url = null; try { const { data } = await sb.storage.from(eqByJob[jid][0].storage_bucket || 'job-photos').createSignedUrl(eqByJob[jid][0].storage_path, 3600); url = data?.signedUrl || null; } catch (_) {}
    out.equipment.push({ name: j?.job_type || 'Equipment', date: j?.completed_at || j?.scheduled_at, url, photos: eqByJob[jid].length });
  }
